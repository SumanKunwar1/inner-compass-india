...
import { healingCategories, amuletIntro, type HealingProduct } from "@/data/healingItems";
import { useProducts, addOrder } from "@/lib/shopStore";
...
function HealingItems() {
  const healingProducts = useProducts();
            ...
            {healingProducts.map((p) => {
              ...
              return (
                <article key={p.id} className="group rounded-2xl overflow-hidden bg-card border border-border hover:border-gold hover:shadow-[var(--shadow-warm)] transition flex flex-col">
                  ...
                  >
                    {p.image ? (
                      <img src={p.image} alt={p.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                    ) : (
            ...
            dialog.mode === "details" ? (
              ...
              <div>
                ...
                <PaymentForm
                  ...
                  onRecord={(payload) => addOrder({ ...payload, productId: dialog.product.id })}
                />
              </div>
            )
          )}
  ...
  return (
    <div>
      <div className="relative aspect-[16/9] rounded-xl grid place-items-center overflow-hidden mb-5" style={{ background: gradient }}>
        ...
        <span className="absolute top-3 left-3 text-cream/85 text-xs uppercase tracking-widest drop-shadow">{catName}</span>
        {product.badge && (
        ...
        {!product.image && <Sparkles className="size-16 text-cream/90" strokeWidth={1.1} />}
      </div>
      <div className="flex items-center gap-1 text-gold-deep">
        {Array.from({ length: 5 }).map((_, i) => (
      ...
      {product.descriptionHtml ? (
        <RichText html={product.descriptionHtml} className="text-sm mt-4" />
      ) : (
