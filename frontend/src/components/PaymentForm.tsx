...
type Props = {
  ...
  /**
   * Called with the full submission so the caller can persist it (e.g. as an order).
   * May return a promise; if it rejects, the error is shown and success is not displayed.
   * When it resolves with a `ref`, that reference is shown to the user.
   */
  onRecord?: (payload: {
    amount: number;
    fullName: string;
...
export function PaymentForm({ context = "Payment", itemLabel, presetAmounts, fixedAmount, note, onDone, onRecord }: Props) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const localRef = "BTMC-" + context.slice(0, 3).toUpperCase() + "-" + Math.random().toString(36).slice(2, 7).toUpperCase();
    let ref = localRef;
    try {
      const result = await onRecord?.({
        amount: fixedAmount ?? (Number(values.amount) || 0),
      ...
      if (result && typeof result === "object" && result.ref) ref = result.ref;
    } catch (err) {
