...
import {
  ...
  Loader2, Heart, Sparkles, PartyPopper, Ticket,
} from "lucide-react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { useState } from "react";
import { getCharityEvent } from "@/data/charityEvents";
...
export const Route = createFileRoute("/charity-events/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.event.title} — ${loaderData?.event.city} | BTMC Foundation India` },
      { name: "description", content: loaderData?.event.intro },
    ],
  }),
    ...
    <Layout>
      ...
      <section className="section-y">
        <div className="container-x max-w-3xl">
          <p className="text-xl leading-relaxed text-foreground/85 font-display">{event.intro}</p>
...
function EventAside({ event }: { event: ReturnType<typeof getCharityEvent> & {} }) {
  return (
...
function BookingForm({ event }: { event: ReturnType<typeof getCharityEvent> & {} }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const ref = "BTMC-CE-" + Math.random().toString(36).slice(2, 7).toUpperCase();
    try {
      sessionStorage.setItem("btmc:lastBooking", JSON.stringify({ ref, event: event.slug, ...values, proofName: proof.name }));
    } catch { /* ignore */ }
    reset();
    setProof(null);
    setSubmitting(false);
